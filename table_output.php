<?php
	...
	class TableOutput{
		...
		function getWhereClauseIfFieldInDBStrucutre($whereArray){
			$existingWhereArray = [];
			foreach($whereArray as $wherePair){
				if (in_array($wherePair[0], SQLTables::$DBTableStructure[$this->tableName]))
					array_push($existingWhereArray, $wherePair);
			}
