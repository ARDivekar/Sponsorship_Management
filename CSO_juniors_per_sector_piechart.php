<?php 
    ...
    $Committe_Members_By_Each_Sector = mysql_query("SELECT SO.Sector, count(SO.SponsID) as Number_Of_Junior_SponsReps
...
WHERE (Year=1 OR Year=2) AND Role = 'Sponsorship Representative'
GROUP BY SO.Sector"); 
    ...
    array('label' => 'JuniorSponsReps', 'type' => 'number')
    ...
    while($result = mysql_fetch_assoc($Committe_Members_By_Each_Sector)) {
        ...
        $temp[] = array('v' => (int)$result['Number_Of_Junior_SponsReps']);
        $rows[] = array('c' => $temp);
...
<h1 class="swTitle">Reports</h1>
    ...
    <div id="Juniors_By_Each_Sector_Piechart" style="width: 900px; height: 500px;"></div>
...
<script>
      ...
      function drawChart() {
        ...
        var options = {
          title: 'Junior Sponsorship Representatives by Sector'
        };

        var chart = new google.visualization.PieChart(document.getElementById('Juniors_By_Each_Sector_Piechart'));
         ...
         var selectedItem = chart.getSelection()[0];
          ...
          if (selectedItem) {
            ...
            if(sector=="Music Stores")
                sector="Music_Stores";
            window.open("./reportico/run.php?project=CSO%20Reports&xmlin=SponsRep_"+sector+".xml&target_style=TABLE&user_criteria_entered=1&target_show_group_headers=1&target_show_detail=1&target_show_group_trailers=1&target_show_column_headers=1&target_show_graph=1&execute_mode=EXECUTE&submitPrepare=1")
          }
        }
