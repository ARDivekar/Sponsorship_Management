<?php 
    ...
    $Committe_Members_By_Each_Sector = mysql_query("SELECT CMPStatus, COUNT(CMPStatus) as Counter FROM `Company` GROUP BY CMPStatus"); 
    ...
    while($result = mysql_fetch_assoc($Committe_Members_By_Each_Sector)) {
        ...
        $temp = array();
        $temp[] = array('v' => (string)$result['CMPStatus']);

        // Values of each slice
        $temp[] = array('v' => (int)$result['Counter']);
        $rows[] = array('c' => $temp);
...
<h1 class="swTitle">Reports</h1>
    ...
    <div id="Company_Status_Piechart" style="width: 900px; height: 500px;"></div>
...
<script>
      ...
      function drawChart() {
        ...
        var chart = new google.visualization.PieChart(document.getElementById('Company_Status_Piechart'));
        ...
        function selectHandler() {
         var selectedItem = chart.getSelection()[0];
          ...
          if (selectedItem) {
            ...
            window.open("./reportico/run.php?project=CSO%20Reports&xmlin=Company_Status_"+status+".xml&target_style=TABLE&user_criteria_entered=1&target_show_group_headers=1&target_show_detail=1&target_show_group_trailers=1&target_show_column_headers=1&target_show_graph=1&execute_mode=EXECUTE&submitPrepare=1")
          }
        }
